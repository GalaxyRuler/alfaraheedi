param(
    [string]$ManualQaRoot = "dist\office-addins-manual-qa",

    [string]$ReportPath = "",

    [string]$GatePath = "office-addins\MANUAL_RELEASE_GATES.md",
...
if (-not (Test-Path -LiteralPath $resolvedGatePath)) {
    throw "Office add-ins manual release gate document not found: $resolvedGatePath"
}
...
if ($reportExists) {
    ...
    $completed = [bool](
        $gateHashMatches -and
        -not $hasTodo -and
        $releaseDecision -eq "Sideload QA approved"
    )
}
...
if ($RequireCompleted -and -not $completed) {
    throw "Office add-ins manual QA report is not complete."
}
