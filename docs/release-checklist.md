...
- [ ] For browser-extension pull requests: confirm the CI artifact `alfaraheedi-browser-extension-0.7.0-release-artifacts` contains the upload zip, `RELEASE_MANIFEST.json`, reviewer docs, and selected screenshots
...
- [ ] For browser-extension VM smokes: use the default guest artifact root under `C:\Temp\Alfaraheedi`, or set `ALFARAHEEDI_VM_QA_ROOT` / pass `-QaRoot <guest-path>` when the QA VM needs a different guest path
...
- [ ] Desktop installer bundle folder contains only the recommended setup installer, not stale or raw Tauri `Alfaraheedi_*_x64-setup.exe` files
- [ ] `.\scripts\check-desktop-installer-bundle.ps1`
- [ ] Desktop Windows CI artifact `alfaraheedi-desktop-windows-setup` contains the canonical setup installer
