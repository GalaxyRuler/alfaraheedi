...
| --- | --- | --- | --- |
| textarea/input inline suggestions | Supported textarea fields and `text`, `search`, `email`, `url`, and `tel` inputs show local-first suggestions directly in the field, with deterministic suggestions from the loopback API. | Focused automated fixture tests and browser package validation. | Yes |
| simple contenteditable suggestions | Simple `contenteditable="true"`, empty `contenteditable`, and `contenteditable="plaintext-only"` editors show suggestions only when text projection to DOM ranges is stable. | Projection tests plus controlled browser QA. | Yes |
| stale apply/suggestion handling | Accepted suggestions apply only when the editor identity, projection hash, original text, replacement text, source rule, trusted UTF-16 span, and stable contenteditable DOM range details still match; stale or unanchored suggestions cannot apply unsafely. | `browserExtensionAnchoredApply.test.js` covers wrong editor identity, changed projection hash, repeated originals without trusted spans, stale contenteditable DOM range details, replacement-like input events, and no fallback to the first occurrence. | Yes |
| sensitive-field exclusion | Password, payment, token, API-key, one-time-code, secret, read-only, disabled, hidden, closed-shadow, oversized, unsupported rich-editor, and similar fields are excluded on a best-effort basis without excluding ordinary message composers such as Cc-recipient contexts. | Discovery tests and manual review of representative fields. | Yes |
